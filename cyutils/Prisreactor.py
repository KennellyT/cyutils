...
class Pris_reactor(object):
    ...
    def __init__(self, reactor_name='', reactor_status='', reactor_type='',
        ...
        self.energy_aval_factor_annual = [
        ] if energy_aval_factor_annual is None else energy_aval_factor_annual
...
def reactor_data(url):
    ...
    lifetime = ['ElectricitySupplied', 'EnergyAvailabilityFactor',
                ...
                'LoadFactor']
    capacities = ['ReferenceUnitPower(NetCapacity)',
                  'DesignNetCapacity', 'GrossCapacity', 'ThermalCapacity']
    ...
    reactor.years = [float(i) for i in years]
    reactor.ele_supplied_GWh = [float(i) for i in ele_supplied_GWh]
    reactor.ref_unit_power_MW = [float(i) for i in ref_unit_power_MW]
    reactor.annual_time_online_hr = [float(i) for i in annual_time_online_hr]
...
def write_pris_csv(url):
    ...
    operating_data = list(zip(years, ele_supplied_GWh, ref_unit_power_MW, annual_time_online_hr,
                              op_factor, energy_aval_factor_annual, load_factor_factor_annual))
...
def pris_reactor_operating_factor_plot(reactor, reactor_name):
    ...
    plt.plot(years, op)
    title = str(reactor_name.reactor_name) + ' ' + 'operating factor'
    ymin, ymax = 0.0, 100.0
    plt.ylim(ymin, ymax)
    plt.title(title)
    plt.xlabel('Years')
    plt.ylabel('Operating factor [%]')
    plt.show()
